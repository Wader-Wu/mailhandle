...
APP_JS = APP_JS.replace(
    'let data={groups:[],stats:{},db_path:\"\",last_sync_end:\"\",mailbox_address:\"\",sync_message:\"\",sync_error:false,sync_running:false,count:0};',
    'let data={groups:[],stats:{},db_path:\"\",last_sync_end:\"\",mailbox_address:\"\",sync_message:\"\",sync_error:false,sync_running:false,count:0,llm_status:{available:true,configured:true,failure_mode:\"abort\",model:\"\",error:false,message:\"\"}};',
)
APP_JS = APP_JS.replace(
...
APP_JS = APP_JS.replace(
    ...
    'function setNotice(message,isError){if(!el.workspaceNotice)return;if(message){el.workspaceNotice.hidden=false;el.workspaceNotice.textContent=message;el.workspaceNotice.classList.toggle(\"error\",!!isError)}else{el.workspaceNotice.hidden=true;el.workspaceNotice.textContent=\"\";el.workspaceNotice.classList.remove(\"error\")}}function clearSyncPoll(){if(syncPollTimer){window.clearTimeout(syncPollTimer);syncPollTimer=null}}function scheduleSyncPoll(delay){clearSyncPoll();syncPollTimer=window.setTimeout(function(){loadData()},delay||1500)}function clearNoticeTimer(){if(noticeTimer){window.clearTimeout(noticeTimer);noticeTimer=null}}function clearAutoSync(){if(autoSyncTimer){window.clearTimeout(autoSyncTimer);autoSyncTimer=null}}function scheduleAutoSync(delay){clearAutoSync();autoSyncTimer=window.setTimeout(function(){if(data.sync_running){scheduleAutoSync(60000);return}refreshNow(true)},typeof delay===\"number\"?delay:AUTO_SYNC_INTERVAL_MS)}function getLlmNotice(){const status=data.llm_status||{};const message=String(status.message||\"\").trim();if(message)return{message:message,error:!!status.error};if(status.available===false){const mode=String(status.failure_mode||\"abort\").trim()||\"abort\";return{message:\"LLM interface is unavailable. Failure mode: \"+mode+\".\",error:true}}return{message:\"\",error:false}}function updateSyncNotice(){const message=String(data.sync_message||\"\");const llmNotice=getLlmNotice();if(data.sync_running){dismissedSyncMessage=\"\";clearAutoSync();clearNoticeTimer();setNotice(message||\"Starting sync...\",false);scheduleSyncPoll(1500);return}clearSyncPoll();scheduleAutoSync(AUTO_SYNC_INTERVAL_MS);if(data.sync_error){dismissedSyncMessage=\"\";clearNoticeTimer();const syncMessage=message||\"Sync failed.\";setNotice(llmNotice.message?syncMessage+\" \"+llmNotice.message:syncMessage,true);return}if(llmNotice.message){dismissedSyncMessage=\"\";clearNoticeTimer();setNotice(llmNotice.message,llmNotice.error);return}if(!message){dismissedSyncMessage=\"\";clearNoticeTimer();setNotice(\"\",false);return}if(dismissedSyncMessage===message){setNotice(\"\",false);return}setNotice(message,false);clearNoticeTimer();noticeTimer=window.setTimeout(function(){dismissedSyncMessage=message;setNotice(\"\",false)},4000)}',
)
APP_JS = APP_JS.replace(
